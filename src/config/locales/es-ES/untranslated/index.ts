export default {
  contrasenaIntroducidaIncorrecta: 'La contraseña debe contener por lo menos, una letra mayúscula, un número y algún caracter especial',
  noSeHanEncontradoResultados: 'No se han encontrado resultados',
  usuariosDadosDeAlta: 'Usuarios dados de alta',
  recetasDadasDeAlta: 'Recetas dadas de alta',
  categoriasDadasDeAlta: 'Categorías dadas de alta',
  si: 'Si',
  no: 'No',
  nick: 'Nick',
  fechaNacimiento: 'Fecha de nacimiento',
  residencia: 'Residencia',
  premiumDesde: 'Premium desde',
  premiumHasta: 'Premium hasta',
  fechaCreacion: 'Fecha de creación',
  horaCreacion: 'Hora de creación',
  fechaBaja: 'Fecha de baja',
  ultimaSesion: 'Última sesión',
  rol: 'Rol',
  aceptaEmail: 'Acepta email',
  aceptaTerminos: 'Acepta términos',
  notificaciones: 'Notificaciones',
  activo: 'Activo',
  darDeBaja: 'Dar de baja',
  darDeBajaUsuario: 'Va a dar de baja al usuario "{usuario}". ¿Seguro que desea realizar esta acción?',
  eliminarUsuario: 'Va a eliminar al usuario "{usuario}". ¿Seguro que desea realizar esta acción?',
  aceptaNotificaciones: 'Acepta notificaciones',
  eliminarReceta: 'Va a eliminar la receta "{receta}". ¿Seguro que desea realizar esta acción?',
  origen: 'Origen',
  unidad: 'Unidad',
  raciones: 'Raciones',
  likes: 'Likes',
  instrucciones: 'Instrucciones',
  administracionDeTiposDePesos: 'Administración de tipos de peso',
  alpha2: 'Alpha 2',
  codigoPais: 'Código país',
  'admin-panel': 'Panel de administración',
  'admin-users': 'Administración de usuarios',
  'admin-recipes': 'Administración de recetas',
  'admin-categories': 'Administración de categorías',
  'admin-countries': 'Administración de paises',
  'admin-temperature-categories': 'Administracióin de temperaturas',
  'admin-unit-times': 'Administración de unidades de tiempo',
  'admin-weight-types': 'Administración de tipos de peso',
};