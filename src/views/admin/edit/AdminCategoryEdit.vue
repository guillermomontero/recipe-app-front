...
import { apiGetCategory, apiEditCategoryAdmin } from '../../../config/api/category';
...
const getCategory = async () => {
  try {
    const response = await apiGetCategory(props.categoryID);
...
const editCategoryAdmin = async () => {
  ...
  try {
    await apiEditCategoryAdmin(payload);
    close(true);
  } catch (error) {
    console.log(error);
  }
      ...
      <form class="form" autocomplete="off">
        <div class="form__row">
          <div class="form__col w-100">
            <input type="text" maxlength="50" placeholder=" " id="categoryName" v-model="category.name" class="form__input">
            <label for="categoryName" class="form__label">{{ t('nombre') }}</label>
          </div>
        </div>
        <div class="form__row">
          <div class="form__col w-100">
            <input type="text" maxlength="50" placeholder=" " id="categoryValue" v-model="category.value" class="form__input" readonly>
            <label for="categoryValue" class="form__label">{{ t('valor') }}</label>
          </div>
        </div>
        <div class="form__row--divider"></div>
        <div>
          <button class="btn btn--md  btn--delete mr-1" @click.prevent="close(false)">{{ t('cancelar') }}</button>
          <button class="btn btn--md  btn--accept" @click.prevent="editCategoryAdmin">{{ t('guardar') }}</button>
        </div>
      </form>
    </article>
  </section>
